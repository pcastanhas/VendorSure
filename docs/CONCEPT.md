...
- **Workflow designer:** a visual flowchart canvas with start nodes, process nodes (rectangles), decision nodes (diamonds), and terminal nodes. Compliance composes workflows by clicking + buttons on existing nodes and picking the next block from a popover. **Phase 5.** The Phase 4 Workflows tab is the entry point.
  ...
  - **Promotion-time validation.** A workflow can be left half-wired in Draft (some Decisions missing their second branch, some Start/Process pointing at nothing). The version's `Draft → InService` transition gates on every workflow being well-formed: every non-terminal node has its required children present. Inline incomplete-badge cues during editing tell the user what's blocking promotion.
